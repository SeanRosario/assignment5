...
class Game(Data):
    ...
    def _do_command(self, words):
        """Input variable "words" is the text entered by the user"""
        
        if self.yesno_callback is not None:
            answer = None #This prevents an error in the 3rd if block
            possible_yes_inputs = ["yes", 'y']
            possible_no_inputs = ["no", 'n']
            if words[0] in possible_yes_inputs:
                answer = True
            if words[0] in possible_no_inputs:
                answer = False
